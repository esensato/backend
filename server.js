...
async function criarImagem(conteudo) {
  const client = new MongoClient(uri);
  try {
      const imagemdb = client.db('imagem');
      const fotos = imagemdb.collection('fotos');
      await fotos.insertOne({imagem: conteudo});
  } catch (err) {
    console.log(err);
    return err;
  } finally {
...
async function atualizarPizza(param_pizzaria, param_pizzaid, param_pizza, param_preco, param_imagem) {
  ...
  try {
      const pizzaria = client.db('pizzaria_app_adm_pizzaria');
      const novaPizza = pizzaria.collection('pizza');
      await novaPizza.updateOne({_id: new ObjectId(param_pizzaid)}, { $set: {pizza: param_pizza, preco: param_preco, imagem: param_imagem}});
  } catch (err) {
    console.log(err);
    return err;
  }finally {
...
app.post('/imagem', async (req, res) => {
  ...
  let ret = await criarImagem(req.body.imagem);
  res.send(ret);
...
app.put('/admin/pizza', async (req, res, next) => {
  ...
  console.log(req.body)
  const ret = await atualizarPizza(req.body.pizzaria, req.body.pizzaid, req.body.pizza, req.body.preco, req.body.imagem);
  res.send(ret);
